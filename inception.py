...
class Inception(nn.Module):
    def __init__(self, in_channels, n_filters, kernel_sizes=[9, 19, 39], stride=2, bottleneck_channels=32,
                 activation=nn.ReLU(),
...
class InceptionBlock(nn.Module):
    def __init__(self, in_channels, n_filters=32, kernel_sizes=[9, 19, 39], stride=2, bottleneck_channels=32,
                 use_residual=True,
...
class InceptionTime(nn.Module):
    ...
    def forward(self, X):
        Z = torch.split(X, 120, dim=1)
        Z1 = self.InBlock1(Z[0])
        Z2 = self.InBlock1(Z[1])
        out = torch.add(Z1, Z2)
